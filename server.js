...
app.get('/search', async (req, res) => {
  ...
  const instance = req.query.instance || 'https://searx.be';
  ...
  try {
    const response = await axios.get(searchUrl, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; 3DS-Proxy/1.0)',
        'Accept': 'application/json',
      },
      timeout: 10000,
    });

    // Return only the fields the frontend needs (keeps payload small for 3DS)
    const results = (response.data.results || []).slice(0, 15).map(r => ({
      title:   r.title   || '',
      url:     r.url     || '',
      content: r.content || '',
    }));
